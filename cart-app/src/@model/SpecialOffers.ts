...
export default class SpecialOffer implements ISpecialOffer {
    specialOfferRules : ISpecialOfferRule[] = [ new ButterRule(), new BreadRule(), new CheeseRule() ];
    ...
    calculateOfferPrice(BasketItems :IBasketItem[]) : ISpecialOfferItem[] {
       return map((element: IBasketItem )=> {
            ...
            return {
                id,
                name,
                savingAmount : this.specialOfferRules.find(rule => rule.rule === name)?.CalculateOfferPrice(BasketItems) ?? 0
            }
        }, BasketItems)  
    }
}
